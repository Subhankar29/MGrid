...
MGrid::MGrid(const vector<vector<double>>& metricObjects, const int numberOfPivots, const int numberOfRings)
  ...
  clusters = Cluster::clusterData(metricObjects, 2, 100);
...
void MGrid::pruneClusters(
  ...
  for (auto& outerIt : mapOfPivotToListOfMinMaxDistancesToRings) {
    ...
    for (auto& ringPair : ringMap) {
      ...
      if (querySpaceRingStartIndex == -1 && ringStartDistanceFromPivot <= querySpaceStartPoint && querySpaceStartPoint <= ringEndDistanceFromPivot) {
...
void MGrid::nnSearchAlgorithm(
  ...
  pruneClusters(queryObject, nearestNeighbourDistance, &activeClusters, *mapOfPivotToListOfMinMaxDistancesToRings, pivots);
  ...
  while (i < activeClusters.size()) {
    ...
    if(!activeClusters[i].isPruned) {
      nearestNeighbourIndex = visitCluster(data, queryObject, activeClusters[i], &currentNearestNeighbourDistance);
    ...
    for (auto & distancesOfIndexDataToPivot : distancesOfIndexDataToPivots) {
      ...
      if (isOdd) {
        if (ringSegmentCount != numberOfRings && listOfDataPointsInARingSegment.size() == totalNumberOfPointsInOneSegment) {
          ...
          ringSegmentCount++;
        } else if (ringSegmentCount == numberOfRings && listOfDataPointsInARingSegment.size() ==  totalNumberOfPointsInOneSegment + 1){
          mapOfRingIndexToDataPoints[ringSegmentCount] = listOfDataPointsInARingSegment;
          listOfDataPointsInARingSegment.clear();
        }
      } else {
